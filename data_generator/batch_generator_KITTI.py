    ...
    while True:
        ...
        for image_path in image_paths[
            ...
            if not labels_subdir is None:
                ...
                label_background = np.all(label == background_color,
                                          axis=2)  # Array of shape (height, width) where every background pixel is `True`.
                label_background = np.expand_dims(label_background, -1)  # Array of shape (height, width, 1).
                label = np.concatenate((label_background, np.invert(label_background)),
                                       axis=2)  # Array of shap (height, width, 2) where the first channel is for the background pixels and the second for the road pixels.
